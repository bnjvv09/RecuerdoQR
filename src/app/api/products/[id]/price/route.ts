...
import { esquemaActualizacionPrecio } from '@/schemas/producto';
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { id } = params;
    if (!id) {
      throw new ErrorApp('ID de producto requerido', CodigosError.VALIDATION_ERROR, 400);
    }
    ...
    const validated = esquemaActualizacionPrecio.parse(body);
    ...
    if (error) {
      throw new ErrorApp(`Error al actualizar el precio: ${error.message}`, CodigosError.DATABASE_ERROR, 500);
    }
